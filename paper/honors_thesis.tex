...
(See Table~\label{tab:primary-alcohol-use-full-summary} for a full
...
indicates that there are likely mediating variables in this relationship.
For instance, the price of alcohol may mean that those living below
the poverty line are less inclined to buy alcohol,
and thus less likely to experience an alcohol use disorder.
It may be that the result for the model with alcohol use as
...
and overall stress.
That is, instead of assuming that poverty status
...
and---with this more accurate predictor in hand---
again investigate the link between financial stress
and mental health.
...
Both of the complications mentioned above
---either that financial hardship is is linked to different mental
health effects than the ones used here,
or that financial hardship is a poor proxy for financial stress---
individually or jointly account for the
