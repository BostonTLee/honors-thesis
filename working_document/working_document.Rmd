...
```{r generating-models}
df_list <- list(primary_model_df, model_with_income_df, model_with_state_df)
name_vec <- c("primary", "median_income", "state")
model_list <- vector(mode = "list")
for (i in 1:2) {
  model_list[[name_vec[[i]]]] <- make_lms(df_list[[i]])
}
```
